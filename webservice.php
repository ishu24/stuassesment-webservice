...
$input_params  = json_decode($jsonInput,true);
	...
	$logo="http://localhost/studentassesment/logo/";
	$question="http://localhost/studentassesment/question/";
	$answer="http://localhost/studentassesment/answer/";
	$timetable="http://localhost/studentassesment/timetable/";
	$answersheet="http://localhost/studentassesment/answersheet/";
...
//--------------------------------user Change Password ----------------------------------------------------------------------------------------------------------//
	...
	if(empty($Semestername) ||  empty($Subjectname) || empty($Userid) || empty($Department))
		{
			header('Content-type: application/json');
			echo json_encode(array('status'=>0,'message'=>'Please fill all require fields.'));
		}
